...
These rules win over the OpenSpec commands in `.claude/commands/opsx/`. Those files
are regenerated by `openspec update`, so they can't be edited to match — where they
say "think deeply", "brainstorm multiple approaches" or "use diagrams liberally",
follow this section instead.

**The one exception is `pinpoint-deep-explore`.** It is OpenSpec's original explore,
kept word for word so it can be used when the user asks for it by name. While it runs,
this section does not apply — its diagrams, its several threads and its length are
the point.

The generated `/opsx:explore` is deleted, and two skills of ours replace it:
`pinpoint-lightweight-explore`, the everyday one, written to follow this section; and
`pinpoint-deep-explore`, the generated text copied word for word, so it only runs when
typed by name and `openspec update` can't rewrite it. Both have a Codex copy in
`.agents/skills/`. The generated one returns whenever `openspec update` runs: the
workflow list lives in a global, per-machine config rather than in this repo, and its
`core` profile includes `explore`. Deleting the file could not record that decision,
so CI does: `pnpm check:openspec-workflows` fails unless the generated workflows are
exactly the five this repo keeps — in `.claude/commands/opsx/`, in `.agents/skills/`,
and in `.claude/skills/`, where the CLI used to write them. It reads the working tree
rather than the index, so it fails the moment `openspec update` writes the file,
before it is staged. Both `.claude/` and `.agents/` are versioned, generated files
included, so a change to them also shows up in `git status`; `.gitignore` is never the
fix, because ignoring a file hides it from CI without stopping an agent on that
machine from reading it. Fix the machine, not just the files:
...
something technical has to come up, explain it at a junior developer's level — plain
words, a concrete example, no unexplained terms.

**Ask before going deep on technical detail.** When a subject needs real technical
discussion, say so in one line and ask:

> How photos get stored is worth a technical discussion. Want me to lay out the
> options, or should I decide?

If the user says to decide, pick what's best and state the choice in one line — "I'll
keep photos in Supabase Storage, which is free up to 1 GB" — so they can still
overrule it.

**Keep questions short and simple.** Lead with your recommendation and add an example
when it helps. Describe each option by what changes for the person using the app, not
by what changes in the code.

**Recommend, don't survey.** Give one recommendation. Show alternatives only when the
user asks for them, or when the choice is theirs to make.

**Don't reopen settled decisions.** Once the user decides something, record it in the
change's artifacts and don't ask again.

**Hold findings until the work is done.** Bugs, cleanups and improvements noticed
while working are not raised mid-task. Add each one to `findings.md` in the change's
folder (gitignored, and it moves with the change on archive): a sentence or two on
what is wrong and what the user would notice. Present the list once the change is
applied and archived. The user decides what to do with each; anything they don't take
up is dropped, and the file is deleted.

The exception is a blocker — you can't continue, something would break or lose data,
or a step would cost money. Stop and say so right away.
...
**Apply and archive a change on the same branch**, one after the other, unless the
