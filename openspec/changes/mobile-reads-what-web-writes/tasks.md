...
- [ ] 1.1 Add a workspace component between `index.tsx` and `TripMap`, owning the trip's
      markers, its interest records and the current filter
- [ ] 1.2 Seed that state from the queries when they first become ready, and confirm a
      later re-render does not re-seed — the defect being avoided is an answer reverting
      because a stale query result was captured before the write
- [ ] 1.3 Read the trip's interest and members alongside its markers and cities, joining
      the existing queries rather than chaining a new one
- [ ] 1.4 Resolve the reader's own member id, since every write is attributed to a member
      rather than an account, and null is ordinary — a member exists before the account

## 2. Recording, on the phone

- [ ] 2.1 Add the per-member rows to the marker sheet — own row interactive, everybody
      else's read-only, undecided drawn as its own state rather than as an unfilled "not
      interested"
- [ ] 2.2 Add the visited control to the sheet
- [ ] 2.3 Write optimistically and revert on failure, reporting what went wrong
...
- [ ] 3.1 Add the filter control to the header, opening a sheet rather than taking a
      permanent row
- [ ] 3.2 Build the picker in the sheet: the people on the trip as its entries, plus
      `Nobody has answered yet`, plus hiding visited — the same questions the laptop asks
- [ ] 3.3 Apply the shared predicate to produce one filtered set and feed it to the map,
      so the two platforms cannot disagree about what a trip contains
- [ ] 3.4 State in the header that the view is narrowed whenever it is, with clearing
...
- [ ] 3.5 Say "no places match this filter" when nothing matches, distinctly from the trip
...
- [ ] 4.2 Confirm no file under `packages/` needed changing. **If one did, stop and record
      why in the roadmap** — the portability claim was the reason this change went first,
      and a quiet edit would throw away the finding
