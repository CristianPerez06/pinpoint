...
**Reason**: The rule applies to every screen and is now stated once in `waiting-screens`
as *The waiting screen and the loaded screen are one definition*, with its rationale. The
part particular to the calendar, which parts stand still and that the drawn rows give way
to the places, has moved into *The calendar is drawn before its data arrives*, together
with this requirement's scenario *The data arrives*.

**Migration**: None. Each application's calendar is already one definition drawing both
states.
