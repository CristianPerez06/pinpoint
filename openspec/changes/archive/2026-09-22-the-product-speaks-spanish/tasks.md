...
- [x] 1.1 Build the phone app to a real iOS device and a real Android device and print all
...
- [x] 1.2 If any form fell back to `2026-04-03` or a separator was wrong, add hand-written
      ...
      **Settled by decision, not by device (2026-09-22):** the user judged the risk
      negligible — current Hermes takes its `Intl` data from the platform, which carries
      Spanish — and chose to assume it works. `Intl` is kept, and the existing fallback to
      the stored string stays in case the assumption is ever wrong.

## 2. A second language in the shared source

- [x] 2.1 Add `'es'` to `LANGUAGES` and a `SPANISH` catalogue typed as `Catalogue`, with
...
- [x] 2.2 Translate the conflict sentence and the two others that name the person
      (`password.missing`, `trip.loadFailed`) and verify each reads as a statement rather
      than an instruction addressed to somebody.
- [x] 2.3 Extend `say.test.ts` to resolve every name in both languages and verify no
      entry returns an empty string or the name itself.
- [x] 2.4 Verify `pnpm check:wording` still passes with two catalogues present.

## 3. Days and prices take the language

- [x] 3.1 Give `day-wording.ts` a language argument mapping to a stated locale per
...
- [x] 3.2 Give `price.ts` and `formatMoney` a language argument. Verify `price.test.ts`
      asserts `USD 1200` and `JPY 3800` carry no separator in Spanish, `USD 12.000` does,
      and `USD 32,50` uses a comma.
- [x] 3.3 Move `Free` into the catalogue as a named sentence resolved by each application,
      and verify no package returns the word.
- [x] 3.4 Replace the two hardcoded `toLocaleString('en')` distance formatters in both
...
- [x] 4.1 Add a language cookie read in the root layout beside the theme cookie, seeded
      into a provider, with `<html lang>` set from it. Verify the page source carries
      `lang="es"` when the cookie says Spanish.
- [x] 4.2 Fall back to the browser's `Accept-Language` when nothing is stored, choosing
...
- [x] 4.3 Add the three-way control to the account menu beside the theme control.
      (The theme control lives on the Settings screen, so the language control is there,
      directly under Appearance, on both platforms.) Verify
...
- [x] 5.1 Add `expo-localization`, read once for the initial language only, and a language
...
- [x] 5.2 Add the three-way control to the account sheet. Verify the choice survives a
      cold launch and that changing the device language afterwards does not override it.
- [x] 5.3 Verify a phone set to German, reading a trip in Spanish, shows the same days and
      prices as a laptop set to English reading the same trip in Spanish.

## 6. The applications' own words move out of them

- [x] 6.1 Move every visible string in `apps/web` into the catalogue in both languages,
      including the aria labels and placeholders. Verify by reading each screen in Spanish
      in the browser — every word changed.
- [x] 6.2 Move every visible string in `apps/mobile` the same way, including all
      `accessibilityLabel` and `accessibilityHint` values. Verify by reading each screen in
      Spanish on a device.
- [x] 6.3 Verify with a screen reader on both platforms that no control announces an
...
- [x] 7.1 Add `react/jsx-no-literals` and the attribute rule to both applications' ESLint
...
- [x] 7.2 Write down what the rule permits, keeping it to punctuation and separators, and
      verify the list is short enough to read at a glance.
- [x] 7.3 Verify `pnpm lint`, `pnpm lint:mobile` and `pnpm lint:packages` all pass with the
      rule on, and that nothing was suppressed to get there.
- [x] 7.4 Add the rule to the CI workflow if it needs its own step, and verify `pnpm verify`
      covers everything CI runs.

## 8. Look at it

- [x] 8.1 Read every screen of the phone app in Spanish on a real device and check the
...
- [x] 8.2 Read every screen of the laptop app in Spanish at 900px, 1080px and full width,
      and check the chrome bar, the filter trigger's pinned width, and the place form.
- [x] 8.3 Switch language with a place's details open on both platforms and verify the
      place's own name and note are unchanged while everything around them is not.
- [x] 8.4 Verify the map attribution reads identically in both languages.
- [x] 8.5 Run `pnpm verify` and `openspec validate the-product-speaks-spanish --strict`.
