...
Where the application offers a way to select the city being worked on, the default
SHALL be that selection. Where it does not, the default SHALL be the city most
recently used on that device. An application SHALL NOT be required to offer city
selection in order to satisfy this specification.
...
#### Scenario: Saving where no city can be selected

- **WHEN** a person saves a place on an application offering no city selection
- **THEN** the form defaults to the city they last used on that device
- **AND** they can change it before saving
