...
- **TypeScript is one major behind what Expo expects.** `expo install --check` reports `typescript@5.9.3 - expected version: ~6.0.3`. Both apps and all three packages typecheck cleanly on 5.9.3, and Next 16's support for TypeScript 6 was not verified. → Accepted deliberately: a compiler major bump is its own change with its own blast radius, not a line item in a skeleton. The cost is a standing advisory from `expo install --check`. Revisit once Next's TypeScript 6 support is confirmed, and move both apps together.
